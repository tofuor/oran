...
#            xapp_port = self.check_port(random.randint(8000,65535))
    ...
    def control_request(self,xApp_ID, SocketFD, DL_MCS=0, UL_MCS=0,
                 DL_maxHARQ=0, UL_maxHARQ=0, repetition=0, Grandfree=0 ):

        if DL_MCS == DL_maxHARQ == UL_MCS == UL_maxHARQ == repetition == Grandfree == 0:
            ...
            controlmessage = json.dumps({
                ...
                "RIC Control Message":{'MCS_DL':DL_MCS,
                                        'MCS_UL':UL_MCS,
                                        'max_DL_HARQ':DL_maxHARQ,
                                        'max_UL_HARQ':UL_maxHARQ,
                                        'Repetition':float(repetition),
